...
Row computeHistogramRAD(Matrix& input, int frames, int index);
void rad(const string& inputDirectory, bool train);

// ------- Main Function --------- //
void rad(const string& originalDirectory, bool train) {
    ...
    for(size_t i = 0; i< result.size(); ++i){
        ...
        for (int i = 0; i < 5; i++) {
            tempDistH = computeHistogramRAD(temp_dist_array, frames, i);  // Compute Histogram for Individual Distances
        ...
        for (int i = 0; i < 5; i++) {
            tempAngH = computeHistogramRAD(temp_angle_array, frames, i);  // Compute Histogram for Angles
